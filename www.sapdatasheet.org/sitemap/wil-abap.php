...
foreach ($list as $row) {
    ...
    $wilurl = "http://www.sapdatasheet.org" . ABAP_UI_Navigation::GetWilPath($row);
    ...
    if ($row['COUNTER'] > ABAP_DB_CONST::INDEX_PAGESIZE) {
        $urls = ABAP_UI_Navigation::GetWilPaths($row['OBJ_TYPE'], $row['OBJ_NAME'], $row['SRC_OBJ_TYPE'], $row['COUNTER']);
